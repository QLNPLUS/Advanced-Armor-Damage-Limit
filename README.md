...
This standalone project targets Fabric 1.21.1 and requires Java 21.

```shell
./gradlew build
```

The release JAR embeds YiRanExpressionLib 1.0.1 as a nested Fabric dependency
resolved from the deterministic local Maven repository in `libs/maven`. Its classes are not merged into the mod's root package,
